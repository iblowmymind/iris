...
|---|---|---|
| Sparse CHD + COW | Every uncompressed parent has a zero SHA-1 (chdman only computes it when compressing); creating its diff succeeds but reopening the parent relationship fails. | Use a sparse overlay — a parentless CHD holding only written hunks, reading the rest from the base — with a recorded base size and modification time. Commit writes the stored hunks back in place. (A first version copied the whole base container into the overlay; see the CHD integrity notes.) |
...
`src/chunk_store.rs`, `src/nfsudp.rs`, `src/wd33c93a.rs`, and
`src/machine.rs`.
...
| 5 | REX shader warm-up policy | A large persistent profile can compete with shaders needed immediately. | Prioritize on-demand work and cap background warm-up; measure time to first interactive frame. |
