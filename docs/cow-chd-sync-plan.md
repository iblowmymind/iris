...
for sparse overlays on uncompressed CHDs, journaled merge recovery, and the CHD snapshot
...
status line read: flatten-on-exit implemented; Phase 1 COW toggle still pending.
What ships now folds an existing `.diff.chd` back into its base
