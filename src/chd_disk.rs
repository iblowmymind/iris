...
// is rewritten only when a hunk first becomes all-zero (rare) or on flush —
...
impl OverlayFile {
    ...
    /// writes them out when the file is closed. Making either durable means
    /// closing the CHD — see [`ChdHd::flush`].
...
pub struct ChdHd {
    /// `None` only between the close and reopen of a [`Self::flush`], and after
    /// one whose reopen failed — [`Self::backend_mut`] turns that into an error
    /// rather than a panic.
    backend: Option<HdBackend>,
    /// Whether anything has been written since the last successful flush. Keeps
    /// a flush with nothing to do from paying for a close/reopen cycle.
    unflushed: bool,
    sector_size: u32,
    total_bytes: u64,
...
impl ChdHd {
    ...
    fn from_sparse(ov: SparseOverlay, base: &Path, diff: PathBuf, dirty: bool, cow: bool) -> Self {
        ...
        Self {
            backend: Some(HdBackend::Sparse(ov)),
            unflushed: false,
            sector_size,
            total_bytes,
            base_path: base.to_path_buf(),
    ...
    fn backend_mut(&mut self) -> io::Result<&mut HdBackend> {
        self.backend.as_mut().ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::BrokenPipe,
                format!(
                    "{} could not be reopened after a flush; this disk is offline",
    ...
    pub fn is_sparse_overlay(&self) -> bool {
        matches!(self.backend, Some(HdBackend::Sparse(_)))
    }
    ...
    /// How much the overlay holds: hunks for a sparse overlay, or a coarse 1/0
    /// for a parented diff (MAME gives us no per-hunk view of one).
    /// Walks the overlay's whole hunk map, so it is an interactive-status call,
    /// not something to put in a polling loop: ~0.5 ms for a 2 GB disk, ~300 ms
    /// for a 240 GB one.
    pub fn overlay_extent(&self) -> usize {
        match &self.backend {
            Some(HdBackend::Sparse(ov)) => ov.owned_hunk_count().unwrap_or(0),
            _ => usize::from(self.dirty),
        }
    }
    ...
    /// Last valid LBA + 1 for this disk.
    fn sector_count(&self) -> u64 {
        self.total_bytes / u64::from(self.sector_size)
    }
    ...
    pub fn write_sectors(&mut self, lba: u64, data: &[u8]) -> io::Result<()> {
        ...
        if ss == 0 || !data.len().is_multiple_of(ss) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
        ...
        // Validate the whole range before touching anything. Writing
        // sector-by-sector and discovering the tail is out of range part-way
        // would have already modified the sectors before it.
        self.check_range(lba, count as u64)?;

        // Mark the overlay dirty before the I/O, not after: a write that fails
        // half-way has still changed the overlay, and a `dirty` flag that only
        // gets set on success would let a clean exit drop those changes.
    ...
    fn check_range(&self, lba: u64, count: u64) -> io::Result<()> {
        let last = self.sector_count();
        let end = lba.checked_add(count).ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "CHD HD LBA range overflows")
        })?;
        if end > last {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("CHD HD LBA {}..{} is outside the disk's {} sectors", lba, end, last),
            ));
        }
        Ok(())
    }

    /// Push everything written so far to stable storage, by closing the CHD and
    /// reopening it. Answers SCSI SYNCHRONIZE CACHE and a forced-unit-access
    /// write.
    ///
    /// Closing is not an implementation detail we could skip. MAME writes hunk
    /// *data* through to the file as it goes, but keeps the hunk map and the
    /// metadata index in memory and commits them only when the file is closed —
    /// `chd_writes_are_only_durable_once_the_file_is_closed` demonstrates that a
    /// second handle sees neither before then. So an `fsync` of the path would
    /// flush bytes that nothing yet points at: on a crash the data is present and
    /// unreachable, which is indistinguishable from losing the write. Since the
    /// device advertises a write-back cache, the command that empties it has to
    /// mean something, and a close/reopen is the only thing that does.
    ///
    /// Costs a map re-read on reopen — around 0.5 ms for a 2 GB disk, 3 ms at
    /// 18 GB, 34 ms at 240 GB — so it is skipped entirely when no write has
    /// happened since the last flush.
    pub fn flush(&mut self) -> io::Result<()> {
        if !self.unflushed {
            return Ok(());
        }
        // Get the bitmap into the file before it closes, or the hunks it covers
        // come back as "absent" and read through to the base.
        self.persist_pending()?;

        // Closing is what commits the map, so the handle has to go before the
        // reopen. Between the two this disk has no backend; `backend_mut` reports
        // that as an error rather than panicking if the reopen fails.
        let (base, diff, cow) = (self.base_path.clone(), self.diff_path.clone(), self.cow);
        let was = self.backend.take();
        drop(was);

        let reopened = match &diff {
            None => HdImage::open(&base).map(HdBackend::InPlace).map_err(map_err),
            Some(d) => match classify_overlay(d) {
                Ok(ExistingOverlay::Sparse) => {
                    SparseOverlay::attach(&base, d).map(HdBackend::Sparse)
                }
                Ok(ExistingOverlay::Parented) => {
                    HdImage::reopen_diff(&base, d).map(HdBackend::Parented).map_err(map_err)
                }
                Ok(ExistingOverlay::UnlinkableParentless) => Err(corrupt(format!(
                    "overlay {} became unreadable across a flush",
                    d.display()
                ))),
                Err(e) => Err(e),
            },
        ...
        match reopened {
            Ok(b) => {
                ...
                let _ = cow; // mode is already reflected in `diff_path`
                Ok(())
            }
            Err(e) => Err(io::Error::new(
                e.kind(),
                format!("could not reopen {} after a flush: {}", base.display(), e),
            )),
        }
    }

    /// Hand the sparse overlay's presence bitmap to MAME without closing the
    /// file, so it is part of whatever the next close commits.
    fn persist_pending(&mut self) -> io::Result<()> {
        if let Some(HdBackend::Sparse(ov)) = &mut self.backend {
            ov.persist_pending()?;
        }
        Ok(())
    }
...
/// Wraps the reader feeding a CHD rebuild and latches the first error, plus the
/// total it produced.
///
/// libchdman's compression source maps both a read error and an early EOF to
/// "zero-pad the rest" — it has no error channel back to MAME and says as much,
/// leaving the caller to compare a SHA-1 if it cares. [`flatten_diff`] replaces
/// the user's base CHD with its output, so it very much cares: without this, a
/// single unreadable hunk in the base or diff turns into a base full of zeros
/// there, and the original is already gone.
...
/// sizes (so a compressed base stays compressed) and its metadata, via a temp
/// file + atomic rename, then delete the diff.
///
/// Safety: the base is only ever replaced by an atomic rename of a fully-written,
/// fsynced temp file whose source stream was verified complete and error-free,
/// and the diff is deleted only after that rename succeeds. On any error or
/// cancellation the base and diff are left exactly as they were, so the next
/// launch simply reattaches the diff — nothing is lost.
///
...
pub fn flatten_diff(
    ...
    use libchdman_rs::CompressionProgress;

    let base_str = path_str(base)?;

    // Read the base's structure so the rebuilt CHD matches it in codecs,
    // geometry and metadata. Scope the handle so it closes before the rename.
    let (codecs, hunk_bytes, unit_bytes, logical, geom, ident) = {
        ...
        // Every metadata record has to survive the rebuild. GDDD and IDNT are
        // carried explicitly below; anything else we have no way to reproduce,
        // so refuse rather than silently drop it — a rebuild that quietly loses
        // a record has replaced the user's disk with a lossy copy.
        let ident = read_ident(&bchd);
        let unsupported: Vec<String> = info
            .metadata_tags
            .iter()
            .map(|(t, _)| *t)
            .filter(|t| *t != HD_GDDD_TAG && *t != HD_IDNT_TAG)
            .map(tag_name)
            .collect();
        if !unsupported.is_empty() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!(
                    "{} carries metadata this rebuild cannot reproduce ({}). Folding the \
                     overlay in would drop it; commit with chdman instead.",
                    base.display(),
                    unsupported.join(", ")
                ),
            ));
    ...
    let reader = CheckedReader::new(MergedReader::new(merged, sector_size, sector_count));
    let total = logical.max(1);
    let mut cb = |cp: CompressionProgress| {
        progress((cp.bytes_done as f64 / total as f64).min(1.0) as f32);
    };
    // `create_from_reader` consumes the reader, so the latched error has to come
    // back out through a shared cell rather than by inspecting it afterwards.
    let outcome = std::cell::RefCell::new(None);
    let reader = TapReader { inner: reader, outcome: &outcome };
    if let Err(e) = create_from_reader(reader, &tmp, opts, &mut cb, cancel) {
        let _ = std::fs::remove_file(&tmp); // base + diff untouched
        return Err(map_err(e));
    }
    // The source is gone now; check what it reported before trusting the output.
    if let Some((err, bytes)) = outcome.into_inner() {
        if let Some(e) = err {
            let _ = std::fs::remove_file(&tmp);
            return Err(io::Error::new(
                e.kind(),
                format!(
                    "reading {} + {} failed ({e}); the base has NOT been replaced",
                    base.display(),
                    diff.display()
                ),
            ));
        ...
        if bytes != logical {
            let _ = std::fs::remove_file(&tmp);
            return Err(corrupt(format!(
                "rebuilding {} produced {} of {} bytes; the base has NOT been replaced",
                base.display(),
                bytes,
                logical
            )));
...
/// Carries a [`CheckedReader`]'s verdict out past `create_from_reader`, which
/// takes the reader by value and drops it internally.
struct TapReader<'a, R: Read> {
    inner: CheckedReader<R>,
    outcome: &'a std::cell::RefCell<Option<(Option<io::Error>, u64)>>,
}

impl<R: Read> Read for TapReader<'_, R> {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        self.inner.read(out)
    }
}

impl<R: Read> Drop for TapReader<'_, R> {
    fn drop(&mut self) {
        *self.outcome.borrow_mut() = Some((self.inner.error.take(), self.inner.bytes));
    }
}

const HD_GDDD_TAG: u32 = u32::from_be_bytes(*b"GDDD");
const HD_IDNT_TAG: u32 = u32::from_be_bytes(*b"IDNT");

fn tag_name(tag: u32) -> String {
    String::from_utf8_lossy(&tag.to_be_bytes()).into_owned()
}

fn read_ident(chd: &Chd) -> Option<Vec<u8>> {
    chd.read_metadata(HD_IDNT_TAG, 0).ok()
}

/// Temp path for the rebuilt CHD, alongside the base so the rename is atomic.
fn temp_sync_path_for(base: &Path) -> PathBuf {
    with_suffix(base, ".synctmp.chd")
}
...
mod tests {
    ...
    /// Folding a diff back must not quietly strip the base's IDNT record.
    #[test]
    fn flatten_preserves_ident_metadata() {
        let base = unique_base();
        let _ = std::fs::remove_file(diff_path_for(&base));
        ...
        create_from_reader(
            ...
            HdCreateOptions {
                ...
                geometry: None,
                ident: Some(b"IRIS-TEST-IDENT".to_vec()),
            },
            &mut |_| {},
            &|| false,
        )
        ...
        drop(hd);
        flatten_diff(&b, &d, &mut |_| {}, &|| false).unwrap();

        let chd = Chd::open(base.to_str().unwrap(), false, None).unwrap();
        assert_eq!(
            chd.read_metadata(HD_IDNT_TAG, 0).unwrap(),
            b"IRIS-TEST-IDENT",
            "IDNT must survive the rebuild"
        );
        drop(chd);
        cleanup(&base);
    }
    ...
    /// The library behaviour that decides what `flush` has to do: neither a
    /// written hunk nor a written metadata record is visible to a fresh handle
    /// until the writer closes the file, and no amount of `fsync` on the path
    /// changes that — MAME keeps the hunk map and the metadata index in memory.
    /// That is why [`ChdHd::flush`] closes and reopens instead of syncing. If a
    /// future libchdman gains a real flush, this test is the signal that the
    /// close can go.
    #[test]
    fn chd_writes_are_only_durable_once_the_file_is_closed() {
        use libchdman_rs::hd::{format_gddd, HdGeometry};
        let p = unique_base();
        let _ = std::fs::remove_file(&p);
        let geom = HdGeometry { cylinders: 1, heads: 1, sectors: 512, sector_bytes: 512 };
        {
            let mut c =
                Chd::create(p.to_str().unwrap(), geom.logical_bytes(), 4096, 512, [0; 4]).unwrap();
            c.write_metadata(HD_GDDD_TAG, 0, &format_gddd(geom), 1).unwrap();
        }

        let mut w = Chd::open(p.to_str().unwrap(), true, None).unwrap();

        // A hunk write, then an fsync of the file. The data bytes are there — the
        // file has grown — but the map entry pointing at them is not, so nothing
        // can find them.
        w.write_hunk(3, &vec![0x5A; 4096]).unwrap();
        fsync_path(&p).unwrap();
        {
            let r = Chd::open(p.to_str().unwrap(), false, None).unwrap();
            assert_eq!(
                r.hunk_info(3).unwrap().compbytes,
                0,
                "a synced hunk write is still unreachable: the map is in the writer's memory"
            );
        }

        // A metadata write is likewise not visible. (It does push the hunk map out
        // as a side effect of rewriting the header, so the map is checked above,
        // before this point, rather than after.)
        w.write_metadata(OVERLAY_TAG, 0, &[0xAB; 8], 0).unwrap();
        fsync_path(&p).unwrap();
        {
            let r = Chd::open(p.to_str().unwrap(), false, None).unwrap();
            assert!(
                r.read_metadata(OVERLAY_TAG, 0).is_err(),
                "a synced metadata write is not in the on-disk index yet"
            );
        }

        // Closing commits both.
        drop(w);
        {
            let r = Chd::open(p.to_str().unwrap(), false, None).unwrap();
            assert_eq!(r.hunk_info(3).unwrap().compbytes, 4096, "closing committed the map");
            assert_eq!(
                r.read_metadata(OVERLAY_TAG, 0).unwrap(),
                vec![0xAB; 8],
                "closing committed the metadata index"
            );
        }
        let _ = std::fs::remove_file(&p);
    }

    /// `flush` has to make a sparse overlay's writes visible to a fresh handle,
    /// including the all-zero hunks that live only in the presence bitmap — and
    /// leave the disk usable afterwards.
    #[test]
    fn flush_commits_the_overlay_and_keeps_the_disk_open() {
        let base = make_base(1024 * 1024, 0xAA, false);
        let mut hd = ChdHd::open(base.to_str().unwrap(), true).unwrap();
        hd.write_sectors(5, &[0x5A; 512]).unwrap();
        hd.write_sectors(8, &[0x00; 4096]).unwrap(); // bitmap-only presence
        hd.flush().unwrap();

        // A second reader, while our handle is still open, sees both.
        {
            let diff = diff_path_for(&base);
            let ov = OverlayFile::open(&diff).unwrap();
            assert!(ov.owns(5 * 512 / 4096).unwrap(), "the ordinary write is committed");
            assert!(ov.owns(8 * 512 / 4096).unwrap(), "the all-zero write is committed");
        }
        // And the disk still works through the reopened handle.
        assert_eq!(hd.read_blocks(5, 1, 512).unwrap(), vec![0x5A; 512]);
        assert_eq!(hd.read_blocks(8, 8, 512).unwrap(), vec![0x00; 4096]);
        assert_eq!(hd.read_blocks(0, 1, 512).unwrap(), vec![0xAA; 512]);
        hd.write_sectors(9, &[0x77; 512]).unwrap();
        assert_eq!(hd.read_blocks(9, 1, 512).unwrap(), vec![0x77; 512]);
        drop(hd);
        cleanup(&base);
    }

    /// A flush with nothing written since the last one must not pay for a
    /// close/reopen cycle.
    #[test]
    fn flush_is_a_no_op_when_nothing_was_written() {
        let base = make_base(128 * 1024, 0xAA, false);
        let mut hd = ChdHd::open(base.to_str().unwrap(), true).unwrap();
        assert!(!hd.unflushed, "a freshly opened disk has nothing pending");
        hd.flush().unwrap();
        hd.write_sectors(1, &[0x5A; 512]).unwrap();
        assert!(hd.unflushed);
        hd.flush().unwrap();
        assert!(!hd.unflushed, "flush clears the pending flag");
        hd.flush().unwrap();
        assert!(!hd.unflushed);
        // Reads do not make it dirty again.
        hd.read_blocks(1, 1, 512).unwrap();
        assert!(!hd.unflushed);
        drop(hd);
        cleanup(&base);
    }

    /// A read error in the merged source must abort the fold rather than
    /// zero-filling the replacement base. Exercised through `CheckedReader`
    /// directly: libchdman's source adapter has no error channel, so this is the
    /// layer that has to notice.
    #[test]
    fn checked_reader_latches_source_errors() {
        struct Failing {
            left: usize,
        }
        impl Read for Failing {
            fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
                if self.left == 0 {
                    return Err(io::Error::new(io::ErrorKind::Other, "simulated bad hunk"));
                }
        ...
        let mut r = CheckedReader::new(Failing { left: 100 });
        let mut buf = [0u8; 64];
        assert_eq!(r.read(&mut buf).unwrap(), 64);
        assert_eq!(r.read(&mut buf).unwrap(), 36);
        assert!(r.read(&mut buf).is_err());
        assert_eq!(r.bytes, 100);
        let e = r.error.expect("the error is latched, not swallowed");
        assert!(e.to_string().contains("simulated bad hunk"));
    }
