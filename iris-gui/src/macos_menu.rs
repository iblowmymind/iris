//! The macOS system menu bar.
//!
//! iris-gui used to carry its menus in a left-hand column inside the window,
//! which meant the emulated display never had the window to itself. On macOS
//! that column is this: the same [`crate::menus`] description, rendered as a
//! real `NSMenu` in the system menu bar, so the app window holds nothing but
//! the guest's picture.
//!
...
//! next frame and applies it, by which point the menu has closed (which is also
//! what lets a menu item open a file dialog).
//!
//! Everything here runs on the main thread: AppKit requires it, and the click
//! callback arrives from the menu's own tracking loop, which runs inside
//! winit's event loop on that same thread.

use crate::menus::{Action, Item, Menu};
